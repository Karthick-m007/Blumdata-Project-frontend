...
const MyQuotesPage = () => {
    ...
    const getStatusColor = (status) => {
        switch (status.toLowerCase()) {
            case 'pending':
                return 'bg-yellow-200 p-1 px-3 rounded-pill text-xs text-yellow-600'; // Yellow for pending
            case 'approved':
                return 'bg-green-200 p-1  px-3 rounded-pill text-xs text-green-600'; // Green for approved
            case 'rejected':
