            ...
            <tbody>
              {quotes.map(quote => (
                <tr key={quote._id} className="border-t  hover:bg-gray-50">
                  ...
                  <td className="p-4">
                    <span className={`px-3 py- rounded-full text-xs font-medium ${quote.status === 'Approved' ? 'bg-green-100 text-green-700' : quote.status === 'Rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                      {quote.status}
                    </span>
                  </td>
                  ...
                  <td className="p-4 space-x-2">
                    {userRole === 'admin' && quote.status === 'Pending' && (
                      <>
                        <button
                          ...
                          className="px-3 py-1 my-2 bg-green-600 text-white rounded-lg hover:bg-green-700"
                        >
                          Accept
                        </button>
                        <button
                          onClick={() => updateStatus(quote._id, 'Rejected')}
                          className="px-3 py-1 my-2 bg-red-600 text-white rounded-lg hover:bg-red-700"
                        >
                          Reject
                        </button>
                      </>
                    ...
                    {quote.status !== 'Rejected' && (
                      <Link to={`/track-order/${quote._id}`} className="text-blue-700 hover:underline">
                        <button className="px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex">
                          Track Order
                        </button>
                      </Link>
                    )}
